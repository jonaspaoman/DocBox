...
export function EventLog({ entries, open, onToggle }: EventLogProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute bottom-full right-0 mb-2 w-[420px] max-h-[320px] overflow-y-auto rounded-md border border-border/40 bg-[oklch(0.12_0_0)] shadow-xl z-50">
          <div className="sticky top-0 bg-[oklch(0.12_0_0)] border-b border-border/30 px-3 py-1.5 flex items-center justify-between">
          ...
          {entries.length === 0 ? (
            ...
            <div className="p-1.5 space-y-px">
              {[...entries].reverse().map((entry) => (
                <div
                  key={entry.id}
                  className="flex items-baseline gap-2 px-2 py-1 rounded hover:bg-white/[0.03] text-xs font-mono"
                >
