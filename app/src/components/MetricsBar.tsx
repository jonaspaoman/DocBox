...
export function MetricsBar({ patients, eventLog, currentTick }: MetricsBarProps) {
  ...
  return (
    <div className="bg-[oklch(0.11_0_0)] px-3 pt-3 pb-3 shrink-0">
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {cards.map((c) => (
          <div
            key={c.label}
            className="flex flex-col items-center rounded-md border border-border/40 bg-[oklch(0.15_0.005_155)] px-1.5 py-2 relative cursor-default transition-colors hover:border-emerald-500/30"
            onMouseEnter={() => setHoveredCard(c.label)}
            onMouseLeave={() => setHoveredCard(null)}
          >
            <span className="text-[9px] font-mono text-white/50 uppercase tracking-wider leading-none mb-1">
              {c.label}
            </span>
            ...
            {hoveredCard === c.label && METHODOLOGY[c.label] && (
              <div className="absolute right-full mr-2 top-1/2 -translate-y-1/2 w-56 px-3 py-2 rounded-md border border-emerald-500/30 bg-[oklch(0.16_0.01_155)] text-[10px] font-mono text-white/80 leading-relaxed z-50 shadow-xl pointer-events-none">
