...
export function PatientRow({ patient, children, onEdit, hideDetails, expanded, onToggle, editableFields, draft, onFieldChange, headerExtra }: PatientRowProps) {
  ...
  return (
    <div className={cn(
      "rounded-md overflow-hidden border-l-2 border border-white/[0.1] bg-white/[0.03] transition-colors",
      expanded && "border-white/[0.15] bg-white/[0.05]",
      ...
      >
        <div ref={contentRef} className="px-4 pb-4 space-y-2 border-t border-white/[0.06] pt-3">
          {!hideDetails && (
            <>
              {/* Demographics bar */}
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs font-mono rounded-md border border-white/[0.06] bg-white/[0.02] p-2.5">
                {editableFields?.has("name") ? (
                  <>
                    <span className="text-muted-foreground/50 uppercase">Name</span>
                    <input className="bg-transparent border-b border-white/20 text-foreground/80 outline-none text-xs font-mono" value={draft?.name ?? patient.name} onChange={(e) => onFieldChange?.("name", e.target.value)} />
                  </>
                ) : null}
                {editableFields?.has("age") ? (
                  <>
                    <span className="text-muted-foreground/50 uppercase">Age</span>
                    <input type="number" className="bg-transparent border-b border-white/20 text-foreground/80 outline-none text-xs font-mono w-16" value={draft?.age ?? patient.age ?? ""} onChange={(e) => onFieldChange?.("age", parseInt(e.target.value) || 0)} />
                  </>
                ) : null}
                {editableFields?.has("sex") ? (
                  <>
                    <span className="text-muted-foreground/50 uppercase">Sex</span>
                    <select className="bg-transparent border-b border-white/20 text-foreground/80 outline-none text-xs font-mono" value={draft?.sex ?? patient.sex ?? ""} onChange={(e) => onFieldChange?.("sex", e.target.value)}>
                      <option value="">—</option>
                ...
                {editableFields?.has("esi_score") ? (
                  <>
                    ...
                    <select className="bg-transparent border-b border-white/20 text-foreground/80 outline-none text-xs font-mono" value={draft?.esi_score ?? patient.esi_score ?? 3} onChange={(e) => onFieldChange?.("esi_score", parseInt(e.target.value))}>
    ...
    <div className="rounded-md border border-white/[0.06] bg-white/[0.02] p-2.5">
      ...
      {editable ? (
        ...
        <p className={cn(
          ...
          highlight === "red" ? "text-red-400" : "text-foreground/80"
        )}>{value}</p>
      )}
    </div>
  );
